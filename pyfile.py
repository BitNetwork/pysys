...
def prompt(text, placeholder, promptColor, inputColor, bgColor):
  ...
  while True:
    draw(paint, inputText, [-1, 3 + len(text)], 0)
    rgbColorTile(paint, [-2, 1], [-1, 1 + len(text)], promptColor, bgColor) # Paint the prompt text
    rgbColorTile(paint, [-2, 0], [-1, -1], inputColor, bgColor) # Paint the status bar
    redraw(False, True)
    # print(inputText)
...
def keyHandle(key): # 27 91 66 - 27 91 65
  ...
  if keypressCache[-3:] == [27, 91, 66]: # down key
    ...
    prompt("Delete file?", "default text", [0xA0, 0xA0, 0xA0], [0x60, 0x60, 0x60], [0xff, 0x0, 0x0])
